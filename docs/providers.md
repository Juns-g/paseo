...
Pi and OMP control-plane RPCs wait 60 seconds by default. OMP's initial `ready` frame waits 10 seconds by default. Override OMP's `params.rpcTimeoutMs` to apply one deadline to both startup and later RPCs when extension or MCP startup on a slow host needs more time. Timeout errors name the pending RPC phase and report both elapsed time and the configured deadline. This setting does not govern long-running Pi compaction or Pi extension UI results.
